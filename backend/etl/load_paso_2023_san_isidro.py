"""ETL bronze -> silver/gold para PASO 2023, circuitos de San Isidro.

Lee el raw descargado por importers/sources/dine/import_paso_2023_san_isidro.py y puebla
core.* y marts.agregados_territoriales. Idempotente: se puede re-ejecutar contra el mismo
directorio de raw sin duplicar filas (usa DELETE + INSERT por elección).

Uso:
    PYTHONPATH=. python3 etl/load_paso_2023_san_isidro.py [--raw-dir data/raw/dine/PASO2023/<timestamp>]
"""
    ...
    con.execute(
        ...
        INSERT INTO core.elecciones
            (id, nombre, tipo, ambito, fecha, anio, categoria_id, categoria_nombre,
             id_eleccion_dine, id_distrito_dine, fuente)
        VALUES (?, ?, 'PASO', 'NACIONAL', DATE '2023-08-13', 2023, ?, 'Presidente/a', ?, 2, 'DINE')
        ON CONFLICT (id) DO NOTHING
        """,
        [ELECCION_ID, "PASO 2023 - Presidente/a", manifest["categoria_id"], manifest["tipo_eleccion"]],
    )
